...
from PIL import Image, ImageTk  # Para cargar imágenes en la galería
...
class App:
    def __init__(self, root):
        ...
        # Crear la carpeta de la galería si no existe
        try:
            os.makedirs(GALLERY_DIR, exist_ok=True)
        except OSError as e:
            messagebox.showerror("Error de Creación de Carpeta", f"No se pudo crear la carpeta '{GALLERY_DIR}':\n{e}")
    ...
    def show_main_menu(self):
        ...
        # Columna Izquierda (primeros 4 simuladores)
        ...
        # Columna Derecha (últimos simuladores y el comparador)
        # Crear botones para los últimos simuladores
        for text, command in simulator_items[4:]:
        ...
        tk.Button(right_frame, text="Comparador de Gráficas EM", command=self.open_comparator, font=button_font, bg="#9a7fdd", fg="white", width=30, height=2, relief="flat", bd=0).pack(pady=10)
    ...
    # --- Método para abrir el comparador ---
    def open_comparator(self):
        self.main_menu.withdraw()
        ComparatorWindow(self.root, self.main_menu)

# --- NUEVA CLASE: Ventana del Comparador de Gráficas ---
...
# --- Clase para la Ventana de Simulación (Modificada) ---
class SimulatorWindow:
    def __init__(self, root, main_menu, dist_type, params):
        ...
        if self.dist_type == "Algoritmo EM (Cáncer)":
            ...
            action_em_frame.pack(pady=10, fill="x") # Fill x
            tk.Button(action_em_frame, text="1. Cargar Archivo", command=self.load_em_data, bg="#007bff", fg="white", relief="flat").pack(fill="x") # Fill x

            column_selection_frame = tk.Frame(top_controls_frame, bg="#2e2e5c")
            column_selection_frame.pack(pady=10, fill="x")

            tk.Label(column_selection_frame, text="2. Variable:", anchor='w', bg="#2e2e5c", fg="white").pack(side="left", padx=5) # Texto acortado

            self.em_column_var = tk.StringVar(self.window)
            self.em_column_var.set("Cargue archivo") # Texto acortado
            self.em_column_var.trace("w", self.on_column_change)

            self.column_menu = OptionMenu(column_selection_frame, self.em_column_var, "")
            self.column_menu.config(bg="#1a1a2e", fg="grey", activebackground="#9a7fdd", relief="flat", highlightthickness=0, state="disabled", width=15) # Ancho fijo
            self.column_menu["menu"].config(bg="#1a1a2e", fg="white")
            self.column_menu.pack(side="left", expand=True, fill="x", padx=5)

            # --- MODIFICADO: Marco para botones horizontales ---
            run_save_frame = tk.Frame(top_controls_frame, bg="#2e2e5c")
            run_save_frame.pack(pady=5, fill="x") # Empaqueta el marco
            ...
            self.run_em_button.pack(side="left", padx=(0,5), fill="x", expand=True) # Empaquetar a la izquierda, expandir
            ...
            self.save_plot_button.pack(side="left", padx=(5,0), fill="x", expand=True) # Empaquetar a la izquierda, expandir
            # --- FIN MODIFICACIÓN ---


            history_frame = tk.Frame(top_controls_frame, bg="#2e2e5c")
            history_frame.pack(pady=5, fill="x") # Fill x
            ...
            self.history_button.pack(side="left", padx=(0,5), fill="x", expand=True) # Expandir
            tk.Button(history_frame, text="Limpiar Historial", command=self.clear_em_history, bg="#f44336", fg="white", relief="flat").pack(side="left", padx=(5,0), fill="x", expand=True) # Expandir
            ...
            tk.Label(checklist_container, text="Superponer Iteraciones:", font=("Helvetica", 12), bg="#2e2e5c", fg="white").pack(anchor='w') # Texto acortado
        ...
        if self.dist_type == "Normal Bivariada":
            ...
            tk.Label(top_controls_frame, text="Resultados:", font=("Helvetica", 12), bg="#2e2e5c", fg="white").pack(pady=(10, 5), anchor='w') # Texto acortado
            self.data_text = scrolledtext.ScrolledText(top_controls_frame, height=10, width=35, bg="#1a1a2e", fg="white", relief="flat") # Ancho ajustado
    ...
    def load_em_data(self):
        try:
            ...
            self.loaded_filename = os.path.basename(filepath).replace(".xlsx", "").replace(".csv", "")
            self.clear_fields()
    ...
    def run_em_algorithm(self):
        try:
            ...
            for i in range(len(iteration_steps)):
                ...
                tk.Label(iter_frame, text=f"Iter {i+1}:", fg="white", bg="#2e2e5c").pack(side="left", padx=5) # Texto acortado
                ...
                var_params = tk.BooleanVar(value=True); cb_params = tk.Checkbutton(iter_frame, text="Val", variable=var_params, command=self.draw_plot, bg="#2e2e5c", fg="#FFD700", selectcolor="#1a1a2e"); cb_params.pack(side="left", padx=5) # Texto acortado
                self.em_iteration_vars.append((var1, var2, var_mezcla, var_params))
                iter_frame.pack(anchor="w", fill="x") # Fill x
    ...
    def save_plot(self):
        if self.df is None or self.fig is None: messagebox.showwarning("Sin Gráfica", "Ejecute el algoritmo primero."); return
        try:
            variable = self.em_column_var.get()
            iters = self.entries["Número de Iteraciones"].get()
            safe_variable = "".join(c if c.isalnum() else '_' for c in variable).rstrip('_')
            safe_iters = "".join(c for c in iters if c.isdigit())
            if not safe_variable: safe_variable = self.loaded_filename
            filename = f"{safe_variable}_{safe_iters}iters.png"
                ...
                for i, (var1, var2, var_mezcla, var_params) in enumerate(self.em_iteration_vars):
                    if var1.get() or var2.get() or var_mezcla.get():
                        ...
                        if var1.get():
                            ...
                            if var_params.get(): peak_y1 = np.max(pdf1) if pdf1.size>0 else 0; peak_x1 = x_curve[np.argmax(pdf1)] if pdf1.size>0 else self.data.min(); param_text1 = f"μ₁={p['mu1']:.2f}\nσ₁={p['sigma1']:.2f}\nπ₁={p['pi1']:.2f}"; self.ax.text(peak_x1,peak_y1,param_text1,color=colors1[i],fontsize=8,va='bottom',ha='center',backgroundcolor=(0,0,0,0.5))
                        if var2.get():
                            pdf2 = stats.norm.pdf(x_curve, p['mu2'], p['sigma2']) * p['pi2']; self.ax.plot(x_curve, pdf2, color=colors2[i], ls=':', label=f'N2 (Iter {i+1})')
                            if var_params.get(): peak_y2 = np.max(pdf2) if pdf2.size>0 else 0; peak_x2 = x_curve[np.argmax(pdf2)] if pdf2.size>0 else self.data.max(); param_text2 = f"μ₂={p['mu2']:.2f}\nσ₂={p['sigma2']:.2f}\nπ₂={p['pi2']:.2f}"; self.ax.text(peak_x2,peak_y2,param_text2,color=colors2[i],fontsize=8,va='bottom',ha='center',backgroundcolor=(0,0,0,0.5))
                        if var_mezcla.get():
    ...
    def show_all_equations_window(self):
        ...
        tk.Label(eq_win, text="Ecuaciones de Mezcla de Normales por Iteración\n  π₁·N(x; μ₁, σ₁²) + π₂·N(y; μ₂, σ₂²)", font=title_font, bg="#1a1a2e", fg="white").pack(pady=(15, 10)) # Formula corregida
